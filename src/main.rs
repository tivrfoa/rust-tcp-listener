...
impl Request {
    fn new(req_type: RequestType, path: String, content_length: usize, body: Option<String>) -> Self {
        Self { req_type, path, content_length, body, headers: HashMap::new() }
    }
				...
				for line in lines {
					...
					if key == "Content-Length" {
						...
						match value.parse::<usize>() {
							...
							Err(e) => return Err(format!("Invalid Content-Length: {}", e.to_string())),
						}
					}
				}
